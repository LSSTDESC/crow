...
def test_evaluates_theory_prediction_returns_value(
    ...
    prediction_exact = binned_exact.evaluate_theory_prediction_counts(
        z_edges, mass_proxy_edges, sky_area, (ClusterProperty.REDSHIFT | ClusterProperty.MASS)
    )
    prediction_grid = binned_grid.evaluate_theory_prediction_counts(
        z_edges, mass_proxy_edges, sky_area, (ClusterProperty.REDSHIFT | ClusterProperty.MASS)
    )
...
def test_evaluates_theory_prediction_with_completeness(
    ...
    prediction_grid = binned_grid.evaluate_theory_prediction_counts( # Grid without completeness
        z_edges, mass_proxy_edges, sky_area
    )
    prediction_grid_w_comp = binned_grid_w_comp.evaluate_theory_prediction_counts( # Grid with completeness
        z_edges, mass_proxy_edges, sky_area
    )
...
def test_setup_clears_caches(binned_grid: GridBinnedClusterRecipe):
    ...
    binned_grid._get_mass_richness_grid(z_test, log_proxy_test, (tuple(z_test), tuple(log_proxy_test)))
...
def test_get_mass_richness_grid(binned_grid: GridBinnedClusterRecipe):
    ...
    recalled_mr_grid = binned_grid._get_mass_richness_grid(z_points, log_proxy_points, key)
...
def test_get_purity_grid(binned_grid: GridBinnedClusterRecipe):
    ...
    binned_grid_w_pur = get_base_binned_grid(
